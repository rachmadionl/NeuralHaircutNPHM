...
class OptimizableCameras(nn.Module):
    ...
    def forward(self, cam_index, intrinsics_0, pose_0):
        ...
        intrinsics = intrinsic_param_to_K(intrinsics_0[:, [0, 1, 0, 1], [0, 1, 2, 2]] + self.intrinsics_res[cam_index])
        rotation_mat = ortho2rotation((self.rotation_res[cam_index] + pose_0[:, :3, :2].transpose(1, 2)).view(bs, -1))

        pose = rotation_mat.new_zeros((bs, 4, 4))
        pose[:, :3, :3] = rotation_mat
        pose[:, 3, 3] = 1
        pose[:, :3, -1] = self.translation_res[cam_index] + pose_0[:, :3, -1]
